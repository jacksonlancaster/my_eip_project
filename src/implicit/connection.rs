...
impl ImplicitConnection {
    ...
    async fn forward_open(client: &Arc<Mutex<AbEipClient>>, rpi_ms: u32) -> Result<(u32, u32)> {
        let mut data = BytesMut::with_capacity(50);
        data.put_u16_le(0x1234);             // Connection Serial Number
        data.put_u16_le(43);                 // Vendor ID (Balluff)
        data.put_u32_le(0x11223344);         // Originator Serial Number
        data.put_u8(4);                      // Timeout Multiplier
        data.put_u8(0); data.put_u8(0); data.put_u8(0); // Reserved
        data.put_u32_le(0);                  // O->T Connection ID (set by device)
        data.put_u32_le(0);                  // T->O Connection ID (set by device)
        data.put_u16_le(0x1234);             // Connection Serial Number (repeat)
        data.put_u16_le(43);                 // Vendor ID (repeat)
        data.put_u8(1);                      // Timeout Ticks
        data.put_u8(0);                      // Priority
    
        let rpi_us = rpi_ms * 1000;
        
        // O->T (Output Assembly 101, size 268 bytes)
        data.put_u32_le(rpi_us);
        data.put_u16_le(268);
       // data.put_u16_le(0x490C);             // Corrected O->T params (0x49 indicates fixed size, 0x0C = 12 bytes)
    
        // T->O (Input Assembly 100, size 394 bytes)
        data.put_u32_le(rpi_us);
        data.put_u16_le(394);
        //data.put_u16_le(0x4931);             // Corrected T->O params (0x49 fixed, 0x31 = 49 bytes)
    
        data.put_u8(1);                      // Transport Class Trigger (Class 1, cyclic)
    
        // Corrected connection path (Config=102, Output=101, Input=100)
        let path = vec![0x20, 0x04, 0x24, 102, 0x2C, 101, 0x2C, 100];
    
        let data_bytes = data.freeze();
        //let mr = MessageRequest::new(0x54, vec![0x20, 0x06, 0x24, 0x01], data_bytes.as_ref());
        let mr = MessageRequest::new(0x54, vec![0x20, 0x06, 0x24, 0x01], data_bytes.as_ref());
